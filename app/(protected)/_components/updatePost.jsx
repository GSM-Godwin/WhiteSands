...
const UpdatePost = ({ post }) => {
  ...
  const [pickupLocation, setPickupLocation] = useState(post.pickupLocation);
  const [dropoffLocation, setDropoffLocation] = useState(post.dropoffLocation);
  ...
  const [numberOfItems, setNumberOfItems] = useState(post.numberOfItems)
  const [name, setName] = useState(post.name);
  const [email, setEmail] = useState(post.email);
  const [phone, setPhone] = useState(post.phone);
  const [isDangerousGoods, setIsDangerousGoods] = useState(post.isDangerousGoods);
  const [isLiveAnimals, setIsLiveAnimals] = useState(post.isLiveAnimals);
  const [isHumanRemains, setIsHumanRemains] = useState(post.isHumanRemains);
  const [totalCost, setTotalCost] = useState(post.price);
  ...
  return (
    <div>
      <form action={editPost} onSubmit={handleSubmit} className="flex flex-col gap-2">
        <label htmlFor="name" className='flex flex-col'>
          Name
          <input type="text" name="name" value={name} onChange={handleInputChange} id="name" placeholder="Name" className="px-2 py-1 rounded-sm border-2 border-gray-500" />
        </label>
        ...
        <button type="submit" disabled={isPending} className={`border-gray-500 border-2 py-2 rounded-sm w-max p-3 ${isPending ? 'cursor-wait border-gray-200 text-gray-200' : ''}`}>{isPending ? 'Updating ...' : 'Update Shipment'}</button>
      </form>
    </div>
  );
}
